...
```bash
# Get state snapshot
node "/Users/lmarques/Dev/efx-mux/.claude/get-shit-done/bin/gsd-tools.cjs" state json 2>/dev/null || echo "{}"
```
...
```bash
node "/Users/lmarques/Dev/efx-mux/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs: defer incomplete Phase {src} items to backlog"
```
