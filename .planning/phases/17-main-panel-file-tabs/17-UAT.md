...
expected: Click a file in the file tree. An editor tab opens in the main panel with file content displayed in CodeMirror 6 with Solarized Dark syntax highlighting.
result: pass
...
result: issue
reported: "First time after app open, Cmd+S saves but closes file. Second time works correctly."
severity: blocker
...
result: issue
reported: "Can drag, green plus icon appears (not a reorder icon), tabs not re-ordered after drop."
severity: major

### 8. [+] Dropdown Menu
expected: Click the [+] button in the tab bar. A dropdown menu appears with three options: Terminal (Zsh), Agent, Git Changes.
result: pass

### 9. Git Changes Accordion
expected: Open the Git Changes tab. An accordion list shows changed files with status badges and +/- line counts. Clicking a file header expands to show inline colored diff.
result: issue
reported: "Line height too big in diff view (too much space between lines). Clicking Git file in left sidebar does not open file in Git Changes tab with expanded accordion."
severity: minor

### 10. Terminal Tabs Still Work
expected: Terminal tabs continue to function normally — creating new terminals, switching between them, typing commands, seeing output.
result: pass
...
- truth: "Drag-and-drop tab reorder moves tab to new position"
  ...
  severity: major
  test: 7
  root_cause: "setProjectTabOrder returns early (if (!name) return) when activeProjectName is null during initialization, silently dropping tab reorder"
  artifacts:
    - path: "src/components/unified-tab-bar.tsx"
      issue: "setProjectTabOrder line 85 returns early when activeProjectName is null"
  missing:
    - "Use placeholder key '_unassigned' instead of early return when activeProjectName is null"
...
- truth: "Cmd+S saves file and keeps editor tab open"
  ...
  severity: blocker
  test: 5
  root_cause: "activeTabId.subscribe guard allows activeUnifiedTabId to be overwritten to terminal when activeUnifiedTabId is empty, hiding editor tab"
  artifacts:
    - path: "src/components/unified-tab-bar.tsx"
      issue: "activeTabId.subscribe guard at line 134 allows sync to terminal when activeUnifiedTabId is empty"
  missing:
    - "Add guard to prevent terminal tab from hijacking focus when editor tab is active"

- truth: "Git Changes accordion shows compact line height matching code editor; clicking sidebar Git file expands that file in accordion"
  status: failed
  ...
  severity: minor
  test: 9
  root_cause: "renderDiffLines uses line-height: 24px (too tall for 13px font); GitFileRow calls openGitChangesTab() with no filePath, no accordion expand mechanism exists"
  artifacts:
    - path: "src/components/git-changes-tab.tsx"
      issue: "renderDiffLines line-height 24px too big; no mechanism to pre-expand specific file"
    - path: "src/components/sidebar.tsx"
      issue: "GitFileRow onClick calls openGitChangesTab() with no filePath argument"
  missing:
    - "Reduce line-height to ~20px in renderDiffLines"
    - "Add openGitChangesTab(filePath) parameter and accordion expand-on-open mechanism"
