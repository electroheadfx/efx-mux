...
// Phase-22 follow-up (quick 260419-k1n): sidebar-main quantization.
// Tests G, H, I for Task 3.
describe('Phase 22 quick-260419-k1n: sidebar-main quantization', () => {
  const MOCK_STATE_SIDEBAR = {
    version: 1,
    layout: {},
    theme: { mode: 'dark' },
    session: {},
  ...
  let ipcSidebarSpy: ReturnType<typeof vi.fn>;

  beforeEach(async () => {
    // Build minimal DOM with sidebar handle, sidebar, and right panel
    document.body.innerHTML = `
      <div id="app">
        <div class="sidebar" style="width: 200px;"></div>
        <div data-handle="sidebar-main" style="width: 4px;"></div>
        <div class="main-panel" style="flex: 1;"></div>
        <div class="right-panel" style="width: 360px;"></div>
      </div>
    `;
    ...
    mockIPC((cmd: string, args: any) => {
      if (cmd === 'load_state') return MOCK_STATE_SIDEBAR;
      if (cmd === 'save_state') { ipcSidebarSpy(args); return undefined; }
      return undefined;
    });
    await loadAppState();
  });
  ...
  it('Test G: snaps sidebar width so residual main width is multiple of cellWidth', () => {
    // main-0 has active terminal with cellWidth=9
    vi.mocked(getCellMetricsForScope).mockImplementation((scope) => {
      if (scope === 'main-0') return { cellWidth: 9, cellHeight: 18 };
      return null;
    });

    // Mock window.innerWidth = 1440
    Object.defineProperty(window, 'innerWidth', { configurable: true, value: 1440 });

    // Mock right panel getBoundingClientRect → width=360
    const rightPanel = document.querySelector<HTMLElement>('.right-panel')!;
    Object.defineProperty(rightPanel, 'getBoundingClientRect', {
      configurable: true,
      value: () => ({ width: 360, left: 1080, right: 1440 } as DOMRect),
    });
    ...
    // User drags to clientX=200 (sidebar width would be 200 raw)
    // residual = 1440 - 200 - 360 = 880
    // snapDown(880, 9, 9) = floor(880/9)*9 = floor(97.78)*9 = 97*9 = 873
    // snappedW = 1440 - 360 - 873 = 207
    // clamped = min(400, max(40, 207)) = 207
    ...
    // Verify CSS var was set to snapped value
    const cssW = document.documentElement.style.getPropertyValue('--sidebar-w');
    expect(cssW).toBe('207px');

    // Verify layout was saved with the snapped value
    expect(ipcSidebarSpy).toHaveBeenCalled();
    const lastArgs = ipcSidebarSpy.mock.calls[ipcSidebarSpy.mock.calls.length - 1][0];
    const saved = JSON.parse(lastArgs.stateJson);
    expect(saved.layout['sidebar-w']).toBe('207px');
  });

  it('Test H: no snap when main-0 has editor active (getCellMetricsForScope returns null)', () => {
    // No active terminal → null → pixel-accurate drag
    ...
    // No snap: CSS var = 200px (raw drag position, clamped but not quantized)
    const cssW = document.documentElement.style.getPropertyValue('--sidebar-w');
    expect(cssW).toBe('200px');

    expect(ipcSidebarSpy).toHaveBeenCalled();
    const lastArgs = ipcSidebarSpy.mock.calls[ipcSidebarSpy.mock.calls.length - 1][0];
    const saved = JSON.parse(lastArgs.stateJson);
    expect(saved.layout['sidebar-w']).toBe('200px');
  });

  it('Test I: clamp floor wins — drag to clientX=30 (below 40) clamps to 40', () => {
    vi.mocked(getCellMetricsForScope).mockImplementation((scope) => {
      if (scope === 'main-0') return { cellWidth: 9, cellHeight: 18 };
      return null;
    });
    ...
    // clientX=30 → raw w = max(40, min(400, 30)) = 40 before snap
    // After snap: residual = 1440 - 40 - 360 = 1040
    // snapDown(1040, 9, 9) = floor(1040/9)*9 = floor(115.5)*9 = 115*9 = 1035
    // snappedW = 1440 - 360 - 1035 = 45
    // clamp: min(400, max(40, 45)) = 45
    // The result may be 45 (clamped residual) but the key behavior is: w >= 40 always
    handle.dispatchEvent(new MouseEvent('mousedown', { bubbles: true, clientX: 0 }));
    ...
    const cssW = document.documentElement.style.getPropertyValue('--sidebar-w');
    const wVal = parseInt(cssW, 10);
    expect(wVal).toBeGreaterThanOrEqual(40);
  });
});
